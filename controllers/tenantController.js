...
const getQuarterlyRentByDueMonth = async (req, res) => {
  try {
    const { estateId } = req.params;
    ...
    const match = {
      ...
      nextDueDate: { $gte: startOfYear, $lt: startOfNextYear },
    };

    if (estateId) {
      // Safely cast estateId for aggregation
      if (!mongoose.Types.ObjectId.isValid(estateId)) {
        return res.status(404).json({ success: false, message: 'Estate not found' });
      }
      match.estate = new mongoose.Types.ObjectId(estateId);
    }

    const rawBuckets = await Tenant.aggregate([
      { $match: match },
      {
        $addFields: {
          month: { $month: '$nextDueDate' },
        },
      },
      {
        $addFields: {
          quarterKey: {
            $switch: {
              branches: [
                { case: { $and: [{ $gte: ['$month', 1] }, { $lte: ['$month', 3] }] }, then: 'Jan-Mar' },
                { case: { $and: [{ $gte: ['$month', 4] }, { $lte: ['$month', 6] }] }, then: 'Apr-Jun' },
                { case: { $and: [{ $gte: ['$month', 7] }, { $lte: ['$month', 9] }] }, then: 'Jul-Sep' },
                { case: { $and: [{ $gte: ['$month', 10] }, { $lte: ['$month', 12] }] }, then: 'Oct-Dec' },
              ],
              default: null,
            },
          },
      ...
      { $match: { quarterKey: { $ne: null } } },
      {
        $group: {
          _id: '$quarterKey',
          totalMonthlyRent: { $sum: '$rentAmount' },
          totalQuarterRent: { $sum: { $multiply: ['$rentAmount', 3] } },
        },
      },
      { $sort: { _id: 1 } },
    ]);

    // Normalize output so all 4 buckets are always present
    const template = [
      { key: 'Jan-Mar', label: 'January - March' },
      { key: 'Apr-Jun', label: 'April - June' },
      { key: 'Jul-Sep', label: 'July - September' },
      { key: 'Oct-Dec', label: 'October - December' },
    ];

    const map = rawBuckets.reduce((acc, row) => {
      acc[row._id] = row;
      return acc;
    }, {});

    const data = template.map((bucket) => ({
      periodKey: bucket.key,
      periodLabel: bucket.label,
      totalMonthlyRent: map[bucket.key]?.totalMonthlyRent || 0,
      totalQuarterRent: map[bucket.key]?.totalQuarterRent || 0,
    }));
    ...
    logError('GET /api/tenants/summary/quarters', err, {
      estateId: req.params.estateId,
      year: req.query?.year,
    });
    return res.status(500).json({
      success: false,
      message: 'Server error occurred while computing quarterly rent summary',
    });
  }
};

// Three-month rent summary for a specific estate and 3‑month window
// Example: /api/estates/:id/three-month-rent?year=2025&startMonth=1
// startMonth is 1–12 (January=1). We always take a 3‑month window
// [startMonth, startMonth+2], e.g. 1 => Jan–Mar, 3 => Mar–May.
const getThreeMonthRent = async (req, res) => {
  try {
    const estateId = req.params.id || req.params.estateId;
    const { year: yearParam, startMonth: startMonthParam } = req.query;

    const now = new Date();
    const year = yearParam ? parseInt(yearParam, 10) : now.getFullYear();
    const startMonth = startMonthParam != null ? parseInt(startMonthParam, 10) : 1;

    if (!Number.isInteger(year) || !Number.isInteger(startMonth)) {
      return res.status(400).json({
        success: false,
        message: 'year and startMonth must be integers',
      });
    }
    ...
    if (startMonth < 1 || startMonth > 12) {
      return res.status(400).json({
        success: false,
        message: 'startMonth must be between 1 (January) and 12 (December)',
      });
    }

    if (!estateId || !mongoose.Types.ObjectId.isValid(estateId)) {
      return res.status(404).json({ success: false, message: 'Estate not found' });
    }

    // Build a 3‑month window: [start, end)
    const startDate = new Date(year, startMonth - 1, 1);
    const endDate = new Date(year, startMonth - 1 + 3, 1);

    const match = {
      isActive: true,
      status: { $in: ['occupied', 'pending'] },
      estate: new mongoose.Types.ObjectId(estateId),
      nextDueDate: { $gte: startDate, $lt: endDate },
    };
    ...
    const [agg] = await Tenant.aggregate([
      { $match: match },
      {
        $group: {
          _id: null,
          totalMonthlyRent: { $sum: '$rentAmount' },
          totalThreeMonthRent: { $sum: { $multiply: ['$rentAmount', 3] } },
          tenantCount: { $sum: 1 },
        },
      },
    ]);
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        startDate,
        endDate,
        totalMonthlyRent: agg?.totalMonthlyRent || 0,
        totalThreeMonthRent: agg?.totalThreeMonthRent || 0,
        tenantCount: agg?.tenantCount || 0,
      },
    });
  } catch (err) {
    logError('GET /api/estates/:estateId/three-month-rent', err, {
      estateId: req.params.id || req.params.estateId,
      year: req.query?.year,
      startMonth: req.query?.startMonth,
    });
    return res.status(500).json({
      success: false,
      message: 'Server error occurred while computing three-month rent summary',
    });
  }
};
