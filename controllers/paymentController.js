  ...
  return async (req, res) => {
    try {
      ...
      const payment = new Payment({
        ...
        amount,
        currency: 'NGN',
        description: description || `${paymentTypeName} for ${tenant.tenantName}`,
        isDeposit,
      ...
      const paymentData = {
        ...
        description: `${paymentTypeName}: ${tenant.tenantName} - Unit ${tenant.unitLabel}`,
...
const verifyPayment = async (req, res) => {
  try {
    ...
    if (!payment) {
      if (redirect) {
        return res.redirect(`/dashboard/payment/success?reference=${reference}&status=error&message=Payment%20record%20not%20found`);
      }
      return res.status(404).json({
        success: false,
    ...
    if (!verificationResult.success) {
      if (redirect) {
        return res.redirect(`/dashboard/payment/success?reference=${reference}&status=error&message=Failed%20to%20verify%20payment`);
      }
      return res.status(400).json({
        success: false,
    ...
    if (verificationResult.status === 'success') {
      ...
      if (redirect) {
        return res.redirect(`/dashboard/payment/success?reference=${reference}&trxref=${reference}&status=success`);
      }
    } else {
      payment.paymentStatus = 'failed';
      ...
      if (redirect) {
        return res.redirect(`/dashboard/payment/success?reference=${reference}&status=failed&message=Payment%20${verificationResult.status}`);
      }
    }
    ...
    if (req.query.redirect) {
      return res.redirect(`/dashboard/payment/success?reference=${reference}&status=error&message=Error%20verifying%20payment`);
    }
