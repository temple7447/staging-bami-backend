...
async def _tenancy_receipt_context(db: AsyncSession, tenant: Tenant, as_of: datetime | None = None) -> dict:
    ...
    return {
        ...
        # The span from the tenant's first year to the year this cycle is
        # in — a single year when it's their 1st year, a range otherwise
        # (2nd year of a tenancy that started in 2025 -> "2025 - 2026").
        "year_duration": (f"{origin.year}" if stay_year == 1
                          else f"{origin.year} - {billing_start.year}"),
